...
let APIClientHandlerErrorDomain = "com.HomeMedics.webserviceerror"
...
class APIClientHandler: TSAPIClient {
    ...
    func sendRequest(_ methodName: String,
        ...
        print("Request URL: ", kStagingBaseUrl + methodName)
        print("Request Headers: ", head ?? [:])
        print("Request parameters: ", Utility.convertDictInJson(parameters ?? [:]))
            ...
            print(result ?? "\nNO Result from API\n")
                ...
                if error?.code == NSURLErrorNotConnectedToInternet {
                    ...
                    if error?.code == NSURLErrorTimedOut{
                        let userInfo : [AnyHashable: Any] = [NSLocalizedDescriptionKey : "Time out! Please try again"]
                        apiError = self.createErrorWithErrorCode(APIClientHandlerErrorCode.noNetwork.rawValue, andErrorInfo: userInfo)
                    }
                ...
                if sendError {
                    ...
                    DispatchQueue.main.async { // Correct
                        completionBlock(response, resultData, resultError, message)
                    }
